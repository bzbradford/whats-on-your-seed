<script>
	...
	/**
	 * @typedef {{ key: string, label: string, sortable?: boolean, thClass?: string, tdClass?: string }} ColDef
	 */

	/** @type {{ rows: any[], columns: ColDef[], sortCol: string, sortDir: string, onSort: (key: string) => void, onRowClick?: (row: any) => void, rowKey: (row: any) => string, cell: import('svelte').Snippet<[ColDef, any]>, emptyMessage?: string }} */
	let {
		rows,
		columns,
		sortCol,
